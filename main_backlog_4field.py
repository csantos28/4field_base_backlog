...
from src.scraper_4field_async import Automation4Field
from src.process_data_4field import ExcelFileHendler
...
class MainOrchestrator:
    ...
    async def _extract_step(self) -> Tuple[bool, Optional[Path], Optional[str]]:
        """Executa a extração (Scraper) e garante o fechamento do browser."""

        scraper = Automation4Field()

        try:
            self.logger.info("🎬 Iniciando extração no 4Field...")
            return await scraper.execute_process_4field()
        finally:
            await scraper.close()
    
    def _load_step(self, file_path: Path, update_time: str) -> bool:
    ...
    async def run_pipeline(self):
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                # Passo 1: Extração
                success_ext, file_path, update_time = await self._extract_step()
                if not success_ext or not file_path:
                    raise RuntimeError("❌ Falha na extração dos dados")
                
                # Passo 2: Carga
                if self._load_step(file_path, update_time):
